...
public record ExtractMixin(String targetClass) {
    ...
    public PatchResult apply(ClassNode classNode, MethodNode methodNode, MethodContext methodContext, PatchContext context) {
        // Sanity check
        ...
        for (MethodNode method : candidates.methods()) {
            ...
            updateOwnerRefereces(method, classNode, this.targetClass);
     ...
     * }</pre>
     */
    private static PatchResult recreateLocalVariables(ClassNode classNode, MethodNode methodNode, MethodContext methodContext, PatchContext context, ClassNode extractClass) {
        ...
        PatchResult result = transform.remover().apply(classNode, methodNode, methodContext, context);
        ...
        TransformParameters cleanupPatch = TransformParameters.builder()
            .chain(b -> IntStream.range(paramLocalStart, paramLocalStart + used.size()).boxed()
                ...
                .forEach(b::remove))
            .build();
        PatchResult cleanupResult = cleanupPatch.apply(classNode, methodNode, methodContext, context);
