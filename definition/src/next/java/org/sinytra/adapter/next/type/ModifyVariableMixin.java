...
public class ModifyVariableMixin implements MixinType<ModifyVariableMixinData> {
    ...
    public ModifyVariableMixinData parse(MixinContext context, ClassTarget targetClass, MethodQualifier targetMethod, AtData atData, AnnotationHandle handle) {
        ...
        Integer ordinal = handle.<Integer>getValue(PROPERTY_ORDINAL).map(AnnotationValueHandle::get).orElse(null);
        SliceData slice = handle.getNested(PROPERTY_SLICE)
            .map(s -> SliceData.parse(s, context))
            .orElse(null);
        return new ModifyVariableMixinData(targetClass, targetMethod, atData, argsOnly, ordinal, slice);
    }
