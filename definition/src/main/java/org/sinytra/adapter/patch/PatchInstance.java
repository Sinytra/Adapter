...
public abstract sealed class PatchInstance implements Patch permits ClassPatchInstance, InterfacePatchInstance {
    public static final Collection<String> KNOWN_MIXIN_TYPES = Set.of(
        MixinConstants.INJECT, MixinConstants.REDIRECT, MixinConstants.MODIFY_ARG,
        MixinConstants.MODIFY_ARGS, MixinConstants.MODIFY_VAR, MixinConstants.MODIFY_CONST,
        MixinConstants.MODIFY_EXPR_VAL, MixinConstants.WRAP_OPERATION, MixinConstants.WRAP_WITH_CONDITION,
        MixinConstants.MODIFY_RETURN_VAL
    );
                ...
                if (methodContext != null) {
                    ...
                    for (MethodTransform transform : this.transforms) {
                        ...
                        if (accepted.isEmpty() || accepted.contains(methodContext.methodAnnotation().getDesc())) {
                            ...
                            if (txResult == Result.APPLY && environment.auditTrail().getMatch(methodContext) == PatchAuditTrail.Match.FULL) {
                                break;
                            }
                        }
                    }
